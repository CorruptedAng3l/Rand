--[[
    ...
    1. SECURITY: Protected GUI Container
       - Using gethui() instead of CoreGui
       - GUI objects hidden from common detection methods
       - Protected from anti-cheat scripts scanning CoreGui/PlayerGui
       
    2. SECURITY: Cloned Service References
       - Using cloneref() for all game services
       - Prevents detection via reference comparison
       - Undetectable service access
       
    3. SECURITY: Caller Protection
       - Added ProtectedCall wrapper function
       - Uses checkcaller() to block external calls
       - Prevents game scripts from accessing library functions
       
    4. PERFORMANCE: Enhanced Drawing Cache
    ...
    5. FILE SYSTEM: Better Error Handling
       - Using isfile() and isfolder() before operations
       - Automatic directory creation in SaveConfig
    ...
    AFFECTED FUNCTIONS:
    ===================
    - Library Init - Added cloneref() for services and gethui() for GUI
    - Utility.AddDrawing() - Added isrenderobj() validation
    - Library.SelfDestruct() - Added cleardrawcache() support
    - Utility.SaveConfig() - Added folder existence checks
    - Utility.DeleteConfig() - Added file existence validation
    - Utility.LoadConfig() - Added file existence checks before reading
    - Utility.AddFolder() - Improved with isfolder() check
    - ProtectedCall() - New security wrapper function
    ...
    - Graceful fallbacks when Potassium functions unavailable
]]
...
-- Security: Caller protection wrapper
...
do
    ...
    Utility.AddDrawing = function(Instance, Properties, Location)
        ...
        --
        -- Validation check using Potassium API
    ...
    Utility.LoadConfig = function(Config)
        local success, err = pcall(function()
            ...
            local LoadedConfig = HttpService:JSONDecode(readfile(filePath))
            --
    ...
    Utility.AddFolder = function(GetFolder)
        local success, err = pcall(function()
            ...
            if not isfolder(GetFolder) then
...
do
    ...
    function Library.SelfDestruct()
        ...
        for _, Value in ipairs(Library.Connections) do
            if Value then Value:Disconnect() end
        end
        --
        -- Use Potassium's cleardrawcache for instant cleanup
    ...
    function Library.Window(Title, Size)
        ...
        local Anime = Utility.AddDrawing("Image", {
            Size = Vector2.new(412, 605),
            Position = Vector2.new(Camera.ViewportSize.X - 400, Camera.ViewportSize.Y - 605),
            Transparency = 0.5, 
            Visible = false,
            Data = ""
        }, Library.Ignores)
        ...
        function Window.ChangeAnime(Name)
            -- Validate that image data exists before assigning
            local imageData = (
            ...
            if imageData and imageData ~= "" then
                ...
                warn("[AbyssLib]: Anime image data not loaded for " .. tostring(Name))
            end
        ...
        function Window:AddSettingsTab(Additional)
            ...
            -- Initialize anime with default character
