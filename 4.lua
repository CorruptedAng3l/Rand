-- CHANGELOG
-- Fixed service caching for better performance (no repeated GetService calls)
-- Fixed Loop function to exit properly on errors (prevents error spam)
-- Fixed RemoveDrawing to iterate backwards (safe removal during iteration)
-- Fixed AddDrawing to validate properties before setting
-- Fixed OnMouse boundary check (>= and <= for accurate detection)
-- Fixed SafeRoundWithDecimals to prevent division by zero
-- Fixed AddDrag to initialize with Vector2.zero for better performance
-- Fixed AddCursor to cache offset calculations and use visibility toggle
-- Fixed SaveConfig/LoadConfig/DeleteConfig with proper error handling
-- Fixed AddImage with error handling for failed downloads
-- Fixed ChangeVisible to batch updates (faster visibility toggling)
-- Fixed SelfDestruct to cleanup all tables and connections properly
-- Fixed Slider bounds validation to use math.clamp
-- Retained full backwards compatibility - all existing code will work
--
local Start = tick()
local LoadTime = tick()
...
local Secure = setmetatable({}, {
    __index = function(Idx, Val)
        local service = game:GetService(Val)
        rawset(Secure, Val, service) -- Cache the service
        return service
    end
})
--
...
do
        ...
        task.spawn(function()
            while task.wait(Delay) do
                ...
                if not Success then 
                    ...
                    break -- Exit loop on error to prevent spam
                end
            end
        end)
    end
    ...
    Utility.RemoveDrawing = function(Instance, Location)
        ...
        --
        for Index = #Location, 1, -1 do -- Iterate backwards for safe removal
            local Value = Location[Index]
            if Value and Value[1] == Instance then
                pcall(function()
                    if Value[1] then
                        Value[1]:Remove()
                    end
    ...
    Utility.AddDrawing = function(Instance, Properties, Location)
        ...
        --
        for Index, Value in pairs(Properties) do
            if Instance[Index] ~= nil then -- Only set valid properties
        ...
        -- Apply text defaults
        if isText then
            Instance.Font = Properties.Font or Library.Theme.Font
            Instance.Size = Properties.Size or Library.Theme.TextSize
        end
        --
    ...
    Utility.OnMouse = function(Instance)
        if not Instance or not Instance.Visible or not Library.WindowVisible then
            return false
        end
    ...
    Utility.SafeRoundWithDecimals = function(Value, Decimals)
        if Decimals == 0 or Decimals == 1 then
            return math.floor(Value + 0.5)
        end
        if Decimals <= 0 then
            return Value -- Prevent division by zero
        end
    ...
    Utility.AddCursor = function(Instance)
        ...
        --
        local mouseCache = Vector2.zero
        local offsetCache = {X5 = 0, X15 = 0, Y5 = 0, Y15 = 0}
        ...
        Utility.AddConnection(RunService.RenderStepped, function()
            ...
            offsetCache.X5 = mouseCache.X + 5
            offsetCache.X15 = mouseCache.X + 15
            offsetCache.Y5 = mouseCache.Y + 5
            offsetCache.Y15 = mouseCache.Y + 15
            
            if not CursorOutline.Visible then
                CursorOutline.Visible = true
                Cursor.Visible = true
            end
            
            CursorOutline.PointA = mouseCache
            CursorOutline.PointB = Vector2.new(offsetCache.X15, offsetCache.Y5)
            CursorOutline.PointC = Vector2.new(offsetCache.X5, offsetCache.Y15)

            Cursor.PointA = mouseCache
            Cursor.PointB = Vector2.new(offsetCache.X15, offsetCache.Y5)
            Cursor.PointC = Vector2.new(offsetCache.X5, offsetCache.Y15)
        end)
    end
    --
    ...
    Utility.SaveConfig = function(Config)
        ...
        if not success then
            warn("[AbyssLib] Failed to save config:", err)
        end
        return success
    end
    --
    Utility.DeleteConfig = function(Config)
        ...
        if not success then
            warn("[AbyssLib] Failed to delete config:", err)
        end
        return success
    end
    --
    Utility.LoadConfig = function(Config)
        local success, LoadedConfig = pcall(function()
            return HttpService:JSONDecode(
                readfile("Abyss/Configs/" .. tostring(game.PlaceId) .. "/" .. Config .. ".json")
            )
        end)
        
        if not success or not LoadedConfig then
            warn("[AbyssLib] Failed to load config:", LoadedConfig)
            return false
        end
        --
    ...
    Utility.AddImage = function(Image, Url, UI)
        ...
        if not success then
            warn("[AbyssLib] Failed to load image:", result)
            return ""
        end
        --
        return ImageFile
            ...
            function Tab:Section(Title, Side)
                ...
                function Section:Slider(Options)
                    ...
                    if Slider.Min > Slider.Max then 
                        Slider.Min, Slider.Max = Slider.Max - 1, Slider.Min
                    end
