--[[
    ...
    - Enhanced code documentation with better comments
    - Renamed services for better readability
    - Added section dividers for easier navigation
    - Improved code organization and structure
    - Performance optimizations for icon caching
    - Better error handling

     Removed PlayerGui fallback (Lines ~652-665):
    - Eliminated xpcall() error handler that fell back to PlayerGui
    - PlayerGui is sometimes actively scanned by anti-cheats
    - Now only uses CoreGui and gethui() for safe GUI parenting
    - Added validation to error out if executor doesn't support safe methods
    - Better to fail safely than risk account bans  
--]]
...
pcall(function()
    if getfenv().getconnections then
        for _, L_33_forvar2 in next, getfenv().getconnections(ScriptContext.Error) do
            pcall(function()
                L_33_forvar2:Disable()  -- Fixed and Issue with Executor Compatibility
            end)
        end
    end
end)
...
-- Safely load icon database with proper error handling
local iconLoadSuccess = pcall(function()
    local httpResult = game:HttpGet("https://raw.githubusercontent.com/ttwizz/Documents/master/icons.json", true)
    if httpResult and httpResult ~= "" then
        local decoded = HttpService:JSONDecode(httpResult)
...
-- If HTTP failed or JSON parsing failed, use the embedded database
if not iconLoadSuccess or type(IconDatabase) == "string" then
    pcall(function()
        IconDatabase = HttpService:JSONDecode(IconDatabase).icons
    end)
end

-- Final fallback to empty table if everything failed
if type(IconDatabase) ~= "table" then
    IconDatabase = {}
end
...
-- Try gethui() for other exploits
elseif getfenv().gethui then
    MainGui.Parent = getfenv().gethui()
-- Fallback to CoreGui (will work on most executors)
else
    MainGui.Parent = CoreGui
end

-- Verify GUI was parented successfully
